...
const OAUTH_REDIRECT_URI = process.env.OAUTH_REDIRECT_URI; // https://cream-bot-messenger.onrender.com/auth
...
app.get("/connect", (req, res) => {
 try {
   ...
   return res.status(500).send(`OAuth config error: ${e.message}`);
 }
});
...
app.get("/auth", async (req, res) => {
 try {
   ...
   // Fetch pages the user can manage (proof OAuth worked)
   ...
   return res.send(`
     <div style="font-family: system-ui; max-width: 720px; margin: 40px auto; line-height:1.4;">
       <h2>✅ Connected</h2>
       <p>OAuth worked. Next step is selecting a page + generating page/IG tokens.</p>
       <h3>Your pages (from /me/accounts)</h3>
       <pre style="background:#f6f6f6; padding:16px; border-radius:8px; overflow:auto;">${escapeHtml(
